...
import { Laptop } from '../laptop/entity/laptop.entity.js';
import { entities } from '../laptop/entity/entities.js';
...
const database = (db?.name as string | undefined) ?? Laptop.name.toLowerCase();

const host = (db?.host as string | undefined) ?? 'localhost';
const username =
    (db?.username as string | undefined) ?? Laptop.name.toLowerCase();
